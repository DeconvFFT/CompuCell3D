    ...
    m_linearIterationsMade(0)
{
	...
	for(unsigned char i=0; i<getFieldsCount(); ++i){
		...
		cl_mem subBuffer=clCreateSubBuffer(mv_newField.handle().opencl_handle().get(), CL_MEM_READ_WRITE, CL_BUFFER_CREATE_TYPE_REGION, &subBufferInfo, &retCode);
...
float NonlinearSolver::Epsilon(viennacl::vector<float> const &v, viennacl::vector<float> const &newField)const{
    ...
    if(!myIsFinite(vn2)){
		...
		sstr<<"Singular Krylov vector passed at linear iteration #"<<m_linearIterationsMade<<". Try to reduce time step.";
...
viennacl::vector<float> const &NonlinearSolver::prod(viennacl::vector<float> const &v_update)const{
	...
	for(unsigned char i=0; i<getFieldsCount(); ++i){
		...
		cl_int retCode;
		cl_mem subBuffer=clCreateSubBuffer(v_update.handle().opencl_handle().get(), CL_MEM_READ_WRITE, CL_BUFFER_CREATE_TYPE_REGION, &subBufferInfo, &retCode);
...
viennacl::vector<float> const &NonlinearSolver::NewField(float dt, viennacl::vector<float> const &v_oldField, NLSParams const &nlsParams){
	...
	for(size_t i=0; i<nlsParams.newton_.maxIterations_; ++i){
		...
		m_linearIterationsMade=0;
		viennacl::linalg::bicgstab_tag solver_tag=viennacl::linalg::bicgstab_tag(nlsParams.linear_.tol_, nlsParams.linear_.maxIterations_);
