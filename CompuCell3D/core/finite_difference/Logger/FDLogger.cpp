...
static FiniteDifference::LoggerCallback callback = NULL;
...
FiniteDifference::LoggingBuffer::LoggingBuffer(int level, const char* func, const char *file, int line):
...
FiniteDifference::LoggingBuffer::~LoggingBuffer()
{
...
std::ostream& FiniteDifference::LoggingBuffer::stream()
{
    return buffer;
}

void FiniteDifference::Logger::setLevel(int level)
{
    logLevel = level;
...
int FiniteDifference::Logger::getLevel()
{
    return logLevel;
}

void FiniteDifference::Logger::disableLogging()
{
    disableConsoleLogging();
    disableFileLogging();
}

void FiniteDifference::Logger::disableConsoleLogging()
{
    consoleStream = NULL;
    if(callback) callback(LOG_OUTPUTSTREAM_CHANGED, consoleStream);
}

void FiniteDifference::Logger::enableConsoleLogging(int level)
{
    setLevel(level);
...
void FiniteDifference::Logger::enableFileLogging(const std::string &fileName, int level)
{
    setLevel(level);
...
void FiniteDifference::Logger::disableFileLogging()
{
...
std::string FiniteDifference::Logger::getCurrentLevelAsString()
{
    return levelToString(logLevel);
}

std::string FiniteDifference::Logger::getFileName()
{
...
std::string FiniteDifference::Logger::levelToString(int level)
{
    switch (level)
    {
    case Message::PRIO_FATAL:
...
FiniteDifference::LogLevel FiniteDifference::Logger::stringToLevel(const std::string &str)
{
...
void FiniteDifference::Logger::log(LogLevel l, const std::string &msg)
{
    FakeLogger &logger = getLogger();
...
void FiniteDifference::Logger::setConsoleStream(std::ostream *os)
{
    consoleStream = os;
...
void FiniteDifference::Logger::setCallback(LoggerCallback cb) {
    callback = cb;
