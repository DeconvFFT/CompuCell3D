 ...
 * @file FDLOGGER.h
 * @author Saumya Mehta.
 * @brief Defines the logger; derived from mechanica FDLOGGER.h originally written by T.J Sego PhD; apparently taken from libRoadRunner rrLogger
 * @date 2021-07-04
 * 
 */
#pragma once
#ifndef SRC_FDLOGGER_H_
#define SRC_FDLOGGER_H_

#ifdef FD_ISWIN
#   define FD_FUNCTION __func__
#else
#   define FD_FUNCTION __PRETTY_FUNCTION__
#endif
...
#define FD_Log(level) \
    if (level > FiniteDifference::Logger::getLevel()) { ; } \
    else FiniteDifference::LoggingBuffer(level, FD_FUNCTION, __FILE__, __LINE__).stream()

#endif /* SRC_FDLOGGER_H_ */